...
Tombstones deserve their own look, because their lifecycle is a trap. A delete does not erase anything — it writes a marker that shadows the key for any read that reaches it. Try it: press **Delete**, then **Read** the same key, and follow the trace. The read stops at the tombstone and reports "not found"; the marker did its job. But the tombstone itself is now data that has to be carried in every file and probed on every read until it reaches the bottommost level and can finally be dropped. Press **Compact** and read the key again — the tombstone is gone, dropped at L1 because L1 is the bottom here, and the read now misses cleanly without it. A workload that deletes heavily can accumulate enough tombstones to inflate both read and space amplification on its own; the marker that makes deletes cheap is itself a cost deferred to compaction.
