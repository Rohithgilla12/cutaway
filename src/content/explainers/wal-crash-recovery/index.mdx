...
import NaiveVsWalViz from "./components/NaiveVsWalViz";
...
<Figure
  number={2}
  label="fsync per page vs append"
  caption="Set rate to 200/s and watch the left queue climb while the right drains."
>
  <NaiveVsWalViz client:visible />
</Figure>
...
<Figure
  ...
  caption="Toggle fsync = off, press Commit a few times, then Crash — watch the lost counter climb."
>
...
A crash is not a clean line. The interesting case is a crash _during_ a flush, when the log's tail is half-written.
...
- Martin Kleppmann, _Designing Data-Intensive Applications_, ch. 3 — write-ahead logs, durability, and the log-structured storage argument.
- Alex Petrov, _Database Internals_, Part I — recovery, redo/undo, ARIES, and checkpointing.
