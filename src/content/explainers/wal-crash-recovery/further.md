...
- **ARIES undo / rollback.** The sim is redo-only: every transaction commits, so
  there is no undo pass. A real recovery has redo (repeating history) *and* undo
...
- **Full-page writes, in depth.** The draft explains *why* they exist (torn
...
- **Group commit internals.** `commit_delay` tuning is genuinely workload-shaped
  (CPU vs storage latency, concurrency). The draft shows the *effect* under Load
...
- Model torn writes that damage a record *other* than the batch's first, to show
...
- Add an undo pass + at least one aborting transaction so the redo/undo split is
  visible.
- Add page LSNs so replay can visibly *skip* a record an already-flushed page
  has seen.
